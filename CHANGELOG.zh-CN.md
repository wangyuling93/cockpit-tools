...
- **网关模式展示为「API 服务」+「新」**：标题与 API 服务卡片统一显示服务名与新网关标识；移除新/旧下拉。
- **精简 Codex 低频入口**：API 服务卡片去掉 CLI 快速启动；账号卡/表格/模型供应商隐藏「服务面板」。
- **Codex API 服务页签栏去掉阴影，统计卡片调整顺序**：图片请求移到末尾；页签文案精简为总览 / Key / 账号 / 模型 / 日志。
- **请求日志隐藏筛选条**：按当前时间范围直接加载。
- **部分 Codex 工具栏精简**：模型供应商与 Codex 多开去掉搜索/筛选；会话页保留类型筛选。
- **主窗口最小宽度下调至 640**：窄屏下胶囊导航改为顶部横栏。
- **服务运行中每 10 秒自动刷新统计**：后台刷新不覆盖正在编辑的端口/代理草稿；横幅可自动关闭也可手动关闭。
...
- **修复较新 Xcode / SPM 布局下 macOS 原生菜单静态库链接失败的问题**：在 `swift-rs` 产物搜索路径之外补充 SPM 配置目录。
- **修复 Codex 从列表行导入 `at-*` access token 时被误识别为 refresh token 的问题**：优先解析嵌入的 personal access token。
