...
- **操作提示 3 秒后自动消失**：「刷新成功」等成功/信息类横幅会自动关闭；错误提示仍需手动关闭。
...
- **网关模式展示为「API 服务」+「新」**：标题与 API 服务卡片统一显示服务名与新网关标识。
- **API 服务卡片去掉 CLI 快速启动按钮**：减少低频入口，卡片操作更精简。
...
- **Codex API 服务页签栏去掉阴影**：总览 / Key / 账号 / 模型 / 日志 顶栏更扁平。
- **统计卡片调整顺序**：将「图片请求」移到总览统计行末尾。
- **网关模式仅展示「新」**：移除 API 服务-新 / 旧下拉与切换引导，统一显示「新」。
- **隐藏「服务面板」入口**：账号卡片、表格操作、API 服务入口与模型供应商卡片上的服务面板按钮不再展示。
...
- **Codex 相关页签文案精简**：账号总览改为「官方」、应用多开改为「多开」、会话管理改为「会话」、模型供应商改为「三方」；Codex API 服务页签改为「总览 / Key / 账号 / 模型 / 日志」。
- **Codex API 服务请求日志隐藏筛选条**：不再展示模型、账号、API 密钥、类型、状态、模式、错误与清除筛选，列表按当前时间范围直接加载。
- **部分 Codex 工具栏精简**：模型供应商、会话管理与 Codex 多开等入口去掉搜索/筛选控件；账号卡片去掉部分低频操作入口，界面更紧凑。
- **主窗口最小宽度下调至 640**：窄屏与分屏时更易容纳。

### 修复

- **修复较新 Xcode / SPM 布局下 macOS 原生菜单静态库链接失败的问题**：在 `swift-rs` 产物搜索路径之外补充 SPM 配置目录，保证 `libMacosNativeMenuSwift.a` 可被找到。
- **修复 Codex 从列表行导入 `at-*` access token 时被误识别为 refresh token 的问题**：优先解析嵌入的 personal access token，再回退到 refresh token。
